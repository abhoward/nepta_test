...
with (o_player_battle_unit) {
	if (poisoned && !poison_hit && instance_exists(o_player_poison_line)) {
		if (unit_ui.x + 3 + ((sprite_get_number(s_action_meter) / max_action_meter) * action_meter) >= o_player_poison_line.x) {
...
with (o_enemy_battle_unit) {
	if (poisoned && !poison_hit && instance_exists(o_enemy_poison_line)) {
		if (unit_ui.x + 3 + ((sprite_get_number(s_action_meter) / max_action_meter) * action_meter) >= o_enemy_poison_line.x) {
